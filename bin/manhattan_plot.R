...
parser$add_argument("--outname", 
                    help = "Output name for the plots (output file prefix)", 
                    default = "DarwinsArk_gp-0.70_biallelic-snps_maf-0.001_geno-0.05_hwe-1e-20-midp-keep-fewhet_N-3465_phe-mq.121_dcov-datatype_qcov-age_rel-cutoff-0.75")
...
data = read_tsv(mlmaFile, col_names = TRUE) %>% as.data.table()
...
if (args$manhattan == TRUE) {
  ...
  # save full as png
  ggsave(filename = paste(outname, ".manhattan.png", sep = ""),
         plot = p,
         device = "png",
         units = "in",
         dpi = 300,
...
if (args$qq == TRUE){
  ...
  ggsave(filename = paste(outname, ".qqplot.png", sep = ""),
         plot = gg.qq,
         device = "png",
         units = "in",
         dpi = 300,
...
if (args$inflation == TRUE) {
  ...
  ggsave(filename=paste(outname, ".inflation.png", sep = ""),
         plot=p,
         device="png",
         width=3*2,
         height=3*2,
  ...
  ggsave(filename=paste(outname, ".inflation.pdf", sep = ""),
         plot=p,
         device="pdf",
         width=3*2,
         height=3*2,
